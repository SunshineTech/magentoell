...
class Fishpig_Wordpress_Model_Observer extends Varien_Object
{
    ...
    /**
     * Hook the video shortcode to the wordpress_shortcode_apply event
     *
     * @param Varien_Event_Observer $observer
     */
    public function applyShortCodes(Varien_Event_Observer $observer)
    {
        $content = $observer->getContent()->getContent();
        $obj     = $observer->getObject();
        Mage::helper('wordpress/shortcode_video')->apply($content, $obj);
        $observer->getContent()->setContent($content);
    }
}
