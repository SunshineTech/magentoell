<?php
/**
 * Mexbs_Tieredcoupon_Model_Resource_Rule_Collection
 * class that is used for overriding the core rule collection
 *
 * @copyright MexBS
 * @author MexBS <it@mexbs.com>
 */
class Mexbs_Tieredcoupon_Model_Resource_Rule_Collection extends Mage_Rule_Model_Resource_Rule_Collection_Abstract
{
    /**
     ...
     * @param bool|null $isTieredCoupon
     * @use $this->addWebsiteGroupDateFilter()
     *
     * @return Mage_SalesRule_Model_Resource_Rule_Collection
     */
    public function setValidationFilter($websiteId, $customerGroupId, $couponCode = '', $now = null, $isTieredCoupon = null)
    {
        if (!$this->getFlag('validation_filter')) {
            ...
            if (strlen($couponCode)) {
                ...
                if($isTieredCoupon){
                    ...
                    if($groupingCoupon->getIsActive()){
                        $subCouponCodes = $groupingCoupon->getSubCouponCodes();
                        if(is_array($subCouponCodes) && (count($subCouponCodes)>0)){
                            $getSubCouponCodesFlag = true;
                            $select->where(sprintf("rule_coupons.code IN (%s))", '"'.implode('","',$subCouponCodes).'"'));
                        }
                    }
                }

                if(!$getSubCouponCodesFlag){
                    $select->where('rule_coupons.code = ?)', $couponCode);
                }
            } else {
