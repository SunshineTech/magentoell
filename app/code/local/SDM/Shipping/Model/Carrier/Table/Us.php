 ...
 */
class SDM_Shipping_Model_Carrier_Table_Us
    extends Mage_Shipping_Model_Carrier_Abstract
    implements Mage_Shipping_Model_Carrier_Interface
{
    const COUNTRY_ID_US = 'US';
     ...
     * @var string
     */
    protected $_code = 'sdm_shipping_table_us';
    ...
    {
        if ($request->getDestCountryId() != self::COUNTRY_ID_US) {
            return false;
        }
        if (!in_array($request->getDestRegionCode(), explode(',', $this->getConfigData('state')))) {
            return false;
        }
    ...
    protected function _getQuotes()
    {
        ...
        if ($total < $this->getConfigData('low_tier_cutoff')) {
            $price = $this->getConfigData('low_tier_price');
        } else {
            $price = $total * $this->getConfigData('high_tier_price_modifier');
        }
        /**
