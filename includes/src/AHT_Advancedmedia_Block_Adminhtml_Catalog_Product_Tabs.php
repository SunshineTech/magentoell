 ...
 */
class AHT_Advancedmedia_Block_Adminhtml_Catalog_Product_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
        ...
        if ($setId) {
            ...
            foreach ($groupCollection as $group) {
				...
				if($group->getAttributeGroupName()=='Images'){
					...
					$this->addTab('advancemedia', array(
						'label'     => Mage::helper('catalog')->__('Video'),
						'content'   => $this->_translateHtml($this->getLayout()->createBlock('advancedmedia/video')->toHtml()),
					));
				}
				else{
					$this->addTab('group_'.$group->getId(), array(
