...
class Fishpig_Wordpress_Model_Post extends Fishpig_Wordpress_Model_Post_Abstract
{
    ...
    /**
     * Get array of meta_value by the postids and metakey
     *
     * @param string postIds and meta_key
     * @return array
     */
    public function getPostMeta($postIds, $meta_key)
    {
        if (!$postIds || !$meta_key) {
            return '';
        }
        $table          = $this->getResource()->getTable('wordpress/post_meta');
        $resource       = Mage::getSingleton('core/resource');
        $readConnection = $resource->getConnection('core_read');
        $query          = "SELECT meta_value FROM {$table} WHERE post_id IN $postIds" . " AND meta_key = '" . $meta_key . "'";
        $data           = Mage::helper('wordpress/database')->getReadAdapter()->fetchAll($query);
        return $data;
    }
}
